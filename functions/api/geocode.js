 ...
 *
 * Geocodifica um endereço usando Nominatim primeiro,
 * com fallback para Mapbox e depois Google Maps.
 *
 * O token Mapbox e a chave do Google NUNCA aparecem no frontend —
 * são lidos exclusivamente do config.json e da variável de ambiente
 * GOOGLE_MAPS_KEY configurada no Cloudflare Pages.
 *
 * Parâmetros:
 *   ?q=Rua+XV+de+Novembro+700   → texto de busca
 *
 ...
 *     "lat": -25.43,
 *     "lon": -49.27,
 *     "display_name": "Rua XV de Novembro, 700, Centro, Curitiba, ...",
 *     "address": { "road": "...", "house_number": "...", "suburb": "...", "city": "..." },
 *     "fonte": "nominatim" | "mapbox" | "google"
 *   }
 * ]
 */
...
async function geocodificarMapbox(query, config) {
  ...
  return data.features
    .filter(f => {
      // Verifica cidade pelo contexto para evitar falsos positivos
      const ctx = f.context || [];
...
// ─────────────────────────────────────────────
// Geocodificação via Google Maps (fallback final)
...
async function geocodificarGoogle(query, config, apiKey) {
  ...
  const params = new URLSearchParams({
    address: `${query}, ${cidade.nome}, PR, Brasil`,
    key: apiKey,
    language: 'pt',
    region: 'br',
